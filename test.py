...
def get_args_parser():
    parser = argparse.ArgumentParser('myMambaNet training and evaluation script for image classification', add_help=False)
    ...
    parser.add_argument('--project', default='mymambanet', type=str,
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume and len(args.resume) > 0:
        ...
        # Load model state dict, skipping head layer weights
        checkpoint_model = checkpoint['model']

        # Remove 'head' parameters from the checkpoint
        ...
        # If an optimizer state exists, load it
        if 'optimizer' in checkpoint and 'epoch' in checkpoint:
            ...
            # Load scaler state for mixed precision training (if used)
...
def main(args):
    ...
    print(f"Training dataset classes: {args.nb_classes}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('myMambaNet training and evaluation script', parents=[get_args_parser()])
